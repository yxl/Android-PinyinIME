...
bool DictBuilder::build_dict(const char *fn_raw,
  ...
  if (kPrintDebug0) {
    printf("homo_idx_num_eq1_: %ld\n", homo_idx_num_eq1_);
    printf("homo_idx_num_gt1_: %ld\n", homo_idx_num_gt1_);
    printf("top_lmas_num_: %ld\n", top_lmas_num_);
  }
...
void DictBuilder::stat_print() {
  ...
  for (size_t i = 0; i < kMaxLemmaSize; i++)
    printf("%ld, ", max_sonbuf_len_[i]);
  printf("-, \n");

  printf(".. max_homobuf_len per layer:\n   -, ");
  for (size_t i = 0; i < kMaxLemmaSize; i++)
    printf("%ld, ", max_homobuf_len_[i]);
  printf("\n");

  printf(".. total_son_num per layer:\n   ");
  for (size_t i = 0; i < kMaxLemmaSize; i++)
    printf("%ld, ", total_son_num_[i]);
  printf("-, \n");

  printf(".. total_node_hasson per layer:\n   1, ");
  for (size_t i = 0; i < kMaxLemmaSize; i++)
    printf("%ld, ", total_node_hasson_[i]);
  printf("\n");

  printf(".. total_sonbuf_num per layer:\n   ");
  for (size_t i = 0; i < kMaxLemmaSize; i++)
    printf("%ld, ", total_sonbuf_num_[i]);
  printf("-, \n");

  printf(".. total_sonbuf_allnoson per layer:\n   ");
  for (size_t i = 0; i < kMaxLemmaSize; i++)
    printf("%ld, ", total_sonbuf_allnoson_[i]);
  printf("-, \n");

  printf(".. total_node_in_sonbuf_allnoson per layer:\n   ");
  for (size_t i = 0; i < kMaxLemmaSize; i++)
    printf("%ld, ", total_node_in_sonbuf_allnoson_[i]);
  printf("-, \n");

  printf(".. total_homo_num per layer:\n   0, ");
  for (size_t i = 0; i < kMaxLemmaSize; i++)
    printf("%ld, ", total_homo_num_[i]);
  printf("\n");

  printf(".. son buf allocation number with only 1 son: %ld\n", sonbufs_num1_);
  printf(".. son buf allocation number with more than 1 son: %ld\n",
         sonbufs_numgt1_);
  printf(".. total lemma node number: %ld\n", total_lma_node_num_ + 1);
}
